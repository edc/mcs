...
def convert_aromatic(graph, flexible=False):
	...
	for line in mol2_lines:
		if line.strip() == '@<TRIPOS>BOND': indices = []
		if indices is not None and len(line.split()) >= 3 and line.split()[3] == 'ar':
...
if __name__ == '__main__':
	...
	try:
		...
		if len(sys.argv) == 4 and sys.argv[3] == '1':
			flex = 2
		g1_size, g2_size, m_size, sub = sdf_mcs(sdffile1, sdffile2, aromatic=flex)
